...
class OnnxWrapper:
    ...
    def __call__(self, x, sr: int):
        ...
        if sr in [8000, 16000]:
            ...
            raise ValueError(
                f"Unsupported sampling rate {sr}. Supported: {self.sample_rates}"
            )
...
def _get_onnx_model_path(
    model_path: str | None = None, opset_version: int = 16
) -> Path:
    available_ops = [15, 16]
    if opset_version not in available_ops:
        raise ValueError(
            f"Unsupported ONNX opset_version: {opset_version}. Available: {available_ops}"
        )

    if model_path is None:
        current_dir = Path(__file__).parent
        data_dir = current_dir.parent / "models" / "silero_vad_models"
        model_name = (
            "silero_vad.onnx"
            if opset_version == 16
            else f"silero_vad_16k_op{opset_version}.onnx"
        )
        model_path = data_dir / model_name
        if not model_path.exists():
            raise FileNotFoundError(
                f"Model file not found: {model_path}. "
                "Place Silero model files under src/models/silero_vad_models/."
            )
    else:
...
def load_jit_vad(model_path: str | None = None):
    if model_path is None:
        current_dir = Path(__file__).parent
        data_dir = current_dir.parent / "models" / "silero_vad_models"
        model_path = data_dir / "silero_vad.jit"
        if not model_path.exists():
            raise FileNotFoundError(
                f"Model file not found: {model_path}. "
                "Place Silero model files under src/models/silero_vad_models/."
    ...
    return init_jit_model(str(model_path))
...
class VADIterator:
    def __init__(
        ...
        if sampling_rate not in [8000, 16000]:
            raise ValueError("VADIterator only supports sampling rates [8000, 16000]")
    ...
    def __call__(self, x, return_seconds: bool = False, time_resolution: int = 1):
        if not torch.is_tensor(x):
            try:
                ...
                raise TypeError(
                    "Audio cannot be cast to tensor. Cast it manually"
                ) from exc
