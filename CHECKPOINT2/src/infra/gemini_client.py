...
class GeminiTopicClient:
    ...
    async def extract_topics(
        ...
        """
        print(f"[DEBUG GeminiTopicClient] extract_topics called: window_chunks_count={len(window_chunks)}, existing_topics_count={len(existing_topics)}")
        if not window_chunks:
            print("[DEBUG GeminiTopicClient] window_chunks is empty, skipping.")
            return None
        ...
        print(f"[DEBUG GeminiTopicClient] calling gemini model={self._model}")
        try:
        ...
        if isinstance(parsed, GeneratedTopicList):
            print(f"[DEBUG GeminiTopicClient] parsed directly from response: topics={len(parsed.topics)}")
            return parsed

        raw_text = getattr(response, "text", None)
        if not raw_text:
            logger.warning("Gemini returned an empty response; skipping window.")
            print("[DEBUG GeminiTopicClient] empty raw_text response from Gemini")
            return None

        print(f"[DEBUG GeminiTopicClient] falling back to parse raw text: {raw_text[:100]}...")
        try:
            parsed = GeneratedTopicList.model_validate_json(raw_text)
            print(f"[DEBUG GeminiTopicClient] parsed from raw text: topics={len(parsed.topics)}")
            return parsed
        except Exception:
