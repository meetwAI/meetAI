...
Persistence
-----------
After every window flush, chunks are embedded with the configured embedder model and written
...
from aiengine.models.chunk import _Sentence
from aiengine.config.settings import settings
from aiengine.models.timed import ASRToken
from aiengine.store.chunks import ChunkStore
from aiengine.clients.gemini import GeminiTopicClient
from aiengine.pipeline.topics import TopicProcessor
from aiengine.pipeline.segmenter import TextSegmenter, _speaker_label
...
class TranscriptionChunker:
    ...
    def __init__(
        ...
        if db_pool is not None and meeting_id and user_id:
            ...
            if (
                ...
                logger.debug(f"Topic extraction enabled, initializing TopicProcessor for meeting_id={meeting_id}")
                ...
                logger.debug(f"Topic extraction enabled BUT missing GEMINI_API_KEY for meeting_id={meeting_id}")
            else:
                logger.debug(f"Topic extraction disabled via settings for meeting_id={meeting_id}")
    ...
    async def _async_flush(self, force: bool = False) -> None:
        ...
        if not new_chunks or self._store is None:
            if not self._store:
                logger.debug("_async_flush: skipped persistence because _store is None")
            return

        persisted = await self._store.persist(new_chunks)
        logger.debug(f"_async_flush: persisted {len(persisted) if persisted else 0} chunks")
        if persisted and self._topic_processor is not None:
            logger.debug(f"_async_flush: sending {len(persisted)} chunks to topic processor")
            await self._topic_processor.add_persisted_chunks(persisted)
        elif self._topic_processor is None:
            logger.debug("_async_flush: skipping topic processor because it is None")
