...
5. Embed each generated topic string with the shared embedder singleton.
...
from aiengine.embedder.embeddings import embed_texts_async
from aiengine.clients.gemini import GeminiTopicClient
from aiengine.store.topics import TopicStore
...
class TopicProcessor:
    ...
    async def add_persisted_chunks(self, chunks: List[PersistedChunk]) -> None:
        ...
        """
        logger.debug(f"add_persisted_chunks called with {len(chunks)} chunks")
        if not chunks:
            return
        ...
        for chunk in chunks:
            ...
            if chunk.end >= window_end:
                logger.debug(f"chunk.end ({chunk.end}) >= window_end ({window_end}). Closing window.")
    ...
    async def flush(self) -> None:
        ...
        if self._pending:
            logger.debug(f"flush forcing close of partial window with {len(self._pending)} chunks")
    ...
    def _close_current_window(self, window_end: Optional[float]) -> None:
        ...
        """
        logger.debug(f"_close_current_window called with window_end={window_end}, pending={len(self._pending)}")
    ...
    async def _process_window_locked(
        ...
        logger.debug(f"_process_window_locked starting for {len(window_chunks)} chunks")
